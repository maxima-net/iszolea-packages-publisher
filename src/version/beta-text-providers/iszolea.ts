...
export class IszoleaBetaTextProvider implements BetaTextProvider {
  ...
  getText(): string | undefined {
    const pbiMatchResult = this.branchName.match(IszoleaBetaTextProvider.PBI_SELECTOR_REGEX);
    if (pbiMatchResult && pbiMatchResult.length >= 1) {
      return `-${pbiMatchResult[0].toLowerCase()}`;
    }

    const sprintMatchResult = this.branchName.match(IszoleaBetaTextProvider.SPRINT_SELECTOR_REGEX);
